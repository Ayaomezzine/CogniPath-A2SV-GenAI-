...
def correct_text():
    ...
    messages = [
        ...
        {
            "role": "assistant",
            "content": "go back in line and provide the corrected paragraph."
        }
    ]
    ...
    try:
        ...
        if "Corrected Text:" in assistant_reply:
            corrected_text = "\n".join(section_contents["Corrected Text:"])
        else:
            corrected_text = "Corrected Text not provided."

    except Exception as e:
        return f"Error generating assistant reply: {str(e)}"

    # Render the HTML template with the variables
    return render_template("EssayCorrection.html", grade=grade, pros=pros, cons=cons, corrected_text=corrected_text, user_input=user_input)
